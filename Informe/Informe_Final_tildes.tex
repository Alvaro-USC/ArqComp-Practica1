...
Los valores de experimentación fueron seleccionados minuciosamente basándose en la arquitectura del procesador. Se identificó el número de líneas límite de memoria dividiendo el tamaño de cada nivel por el tamaño de una línea de caché ($64\ bytes$): $S_1 = 768$ (L1), $S_2 = 20\,480$ (L2) y $S_3 = 786\,432$ (L3). A partir de estos topes, los valores de prueba $L$ se escogieron utilizando fracciones o múltiplos representativos (por ejemplo, $0.5 \times S_1$, $1.5 \times S_1$, $4.0 \times S_3$). Este método garantiza probar escenarios donde los datos residen cómodamente dentro de una caché, desbordan sutilmente al siguiente nivel, o llenan por completo la jerarquía obligando al sistema a acceder a la memoria RAM profunda (hasta 192 MB).
...
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{\texttt{acp1.c} (Base doble indirecto):} Evalúa un arreglo tipo \texttt{double} de 8 bytes con un patrón de acceso indirecto (\texttt{A[ind[i]]}), provocando gran carga sobre la memoria al requerir dos lecturas por iteración.
  \item \textbf{\texttt{acp1\_int.c} (Entero indirecto):} Modifica el arreglo a tipo \texttt{int} de 4 bytes. Como un entero ocupa la mitad que un \texttt{double}, caben el doble de elementos útiles por línea de caché ($R$ se duplica), lo que reduce la latencia en sumas y la presión general de memoria.
  \item \textbf{\texttt{acp1\_directo.c} (Doble directo):} Elimina por completo el arreglo auxiliar \texttt{ind[]} accediendo de forma secuencial explícita (\texttt{A[i * D]}). Esto previene que los índices agoten el espacio en caché y favorece el funcionamiento del prefetcher hardware.
\end{itemize}

\subsection{Análisis de la cantidad de accesos y saltos en caché}
El salto de caché depende estrictamente del tamaño de la línea (\textbf{64 bytes}) y del tipo de dato. Un salto ocurre cuando el patrón de acceso supera el límite de los 64 bytes forzando a cargar una línea diferente.
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Para tipo \texttt{double} (8 bytes):} Caben exactamente 8 elementos en una línea de caché. Si se utiliza un stride $D=1$, el sistema ejecuta \textbf{8 accesos} útiles antes de forzar un salto a la siguiente línea. Si se emplea un stride $D=8$, cada salto avanza 64 bytes ($8 \times 8$), lo que implica que se produce exactamente \textbf{1 único acceso} por línea de caché.
  \item \textbf{Para tipo \texttt{int} (4 bytes):} Caben 16 elementos por línea. Con $D=1$, se efectúan \textbf{16 accesos} antes de requerir una nueva línea. Si usamos un stride de $D=16$, la separación será de $16 \times 4 = 64$ bytes, lo que vuelve a forzar el límite de \textbf{1 único acceso} por línea.
\end{itemize}
